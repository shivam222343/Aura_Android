...
io.on('connection', (socket) => {
    ...
    socket.on('call:start', async (data) => {
        const { recipientIds, type, isGroup } = data;
        ...
        try {
            ...
            console.log(`📞 Call started by ${senderId} to [${recipientIds.join(', ')}]`);
        } catch (error) {
    ...
    socket.on('call:accept', (data) => {
        ...
        io.to(callerId).emit('call:accepted', { acceptorId: recipientId });
    ...
    socket.on('call:busy', (data) => {
        const { callerId } = data;
        const recipientId = socket.userId;
        io.to(callerId).emit('call:busy', { rejectorId: recipientId });
    });
    ...
    socket.on('call:offer', (data) => {
        io.to(data.to).emit('call:offer', { offer: data.offer, from: socket.userId });
    });
    socket.on('call:answer', (data) => {
        io.to(data.to).emit('call:answer', { answer: data.answer, from: socket.userId });
    });
    socket.on('call:ice-candidate', (data) => {
        io.to(data.to).emit('call:ice-candidate', { candidate: data.candidate, from: socket.userId });
    });
